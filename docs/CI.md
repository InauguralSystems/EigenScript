...
with PyYAML and requires a top-level `jobs:` mapping. An unloadable file is
red by name. A missing PyYAML, or zero files, is red — a skip is not a pass.
A load does not prove GitHub's workflow schema accepts the file. Linux legs
get `python3-yaml` from `.devcontainer/Dockerfile`; the macOS job installs
PyYAML and fails the job if the import fails, so the load runs there too.
...
self-tests` job. A missing PyYAML is exit 2, never a pass.
