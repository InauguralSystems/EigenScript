...
- **CI uses the full suite on each binary and a sharded ASan HTTP/model build
  (#1160, #1275).** Every variant job runs `tests/run_all_tests.sh` on PRs and
  main. The former per-variant section plans saved no PR wall time: those jobs
  already finished before the gcc and ASan critical path. Their environment
  selector, capability probes and markers, and gate-audit machinery are gone.
  - `make asan-http` builds the sanitizer binary with HTTP, model, and network
    extensions. `EIGS_SUITE_SHARD=k/N` runs one weight-balanced part of its full
  ...
  - `tools/section_plan.sh --skip-audit` checks every section-level skip
    emitter against `section_skip` routing or a reviewed waiver. New bare
    `SKIP:` lines are refused; `--selftest` plants that fault and the other
    surviving shard-gate faults in copies. The audit no longer needs a
    capability gate inventory because no variant plan can omit one.
  - The generated-LSP-header probes in [99i] run uncached; its expensive
    warning-error dry run is cached by `tools/werror_cache_key.sh`. Suite jobs
    use `EIGS_SKIP_WERROR_AUDIT=1` and print the owning job, while a local run
    with the variable unset performs both halves.
  - `macos-15-intel` and the full valgrind corpus run nightly; macOS also
    runs on main, and PRs and main keep the fixed valgrind smoke. `docs/CI.md`
    maps the PR, main, and nightly lanes and their required checks.
...
- **The playground's real wasm32 build runs on every pull request (#1255).**
  ...
  has its own concurrency group so it cannot cancel a `main` deploy. The host
...
- **The Value-union size assert is the #1183 claim that holds at every
  ...
  bytes to the union: `sizeof(data.strv) <= sizeof(data.fn)`. Gated by the
  real wasm32 build, `playground (real emcc wasm32 build)`, required on every
  pull request.
