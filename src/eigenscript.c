...
void arena_init(void) {
    ...
    g_arena.active = 0; /* off by default, enabled by arena_mark builtin */
    ...
    if (g_arena.offset + size > ARENA_BLOCK_SIZE) {
        ...
        if (g_arena.current_block >= g_arena.block_count) {
            if (g_arena.block_count >= ARENA_MAX_BLOCKS) {
                /* Out of arena blocks, fall back to calloc */
...
static void arena_track_string(char *s) {
...
void arena_reset_to_mark(void) {
    /* Free strings allocated since mark */
    ...
    /* Reset arena position — no need to free blocks, just rewind */
    g_arena.current_block = g_arena.mark_block;
    g_arena.offset = g_arena.mark_offset;

    /* Deactivate arena — inter-step allocations go to heap.
     * Next arena_mark will re-activate. */
    g_arena.active = 0;
}
...
/* Free a VAL_NUM that was heap-allocated (make_num_permanent).
 * Safe to call on weight matrix elements being replaced by sgd_update
 * or perturbation temporaries being discarded by numerical_grad.
 * make_num_permanent always uses calloc (never arena), so these Values
 * are safe to free regardless of whether the arena is currently active.
 * Does NOT free arena-allocated or non-numeric Values. */
static void free_weight_val(Value *v) {
    if (!v || v->type != VAL_NUM) return;
    /* Only free if the pointer is outside the arena blocks.
     * make_num_permanent Values are always heap-allocated. */
    for (int i = 0; i < g_arena.block_count; i++) {
        char *block_start = g_arena.blocks[i];
        char *block_end = block_start + ARENA_BLOCK_SIZE;
        if ((char*)v >= block_start && (char*)v < block_end) return; /* arena Value, don't free */
    }
    free(v);
}
