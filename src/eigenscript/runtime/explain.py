...
class PredicateExplainer:
    ...
    def explain_stable(
        ...
        print(
            f"  └─ stable dimensions: {stable_dims}, changing: {changing_dims}",
            file=sys.stderr,
        )
    ...
    def explain_converged(
        ...
        print(
            f"  └─ framework_strength = {fs:.4f} (threshold: {threshold})",
            file=sys.stderr,
        )
    ...
    def explain_improving(
        ...
        if trajectory_length < 2:
            print(
                f"  └─ insufficient trajectory data (length: {trajectory_length})",
                file=sys.stderr,
            )
    ...
    def explain_oscillating(
        ...
        if trajectory_length < 5:
            print(
                f"  └─ insufficient trajectory data (length: {trajectory_length}, need: 5)",
                file=sys.stderr,
            )
        else:
            comparison = ">" if result else "≤"
            print(
                f"  └─ oscillation_score = {oscillation_score:.3f} (threshold: {threshold})",
                file=sys.stderr,
            )
