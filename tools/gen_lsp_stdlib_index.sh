#!/bin/bash
# Generate src/lsp_stdlib_index.h — the stdlib-module index the LSP
# serves as import-aware completion + hover (#590).
#
# Scrapes every stdlib module for PUBLIC `define name(params)` definitions
...
BEGIN {
    ...
    print " * Also emits stdlib_modules[]: every stdlib-module basename, which is what" > OUT
    print " * `import ` completion offers (#692). That list is deliberately NOT" > OUT
    print " * derived from stdlib_docs -- a module whose defines are all private" > OUT
    print " * (ui_layout) or which declares none at top level (text_builder) has no" > OUT
    print " * row there but is still perfectly importable, so deriving it would" > OUT
    print " * silently drop 5 real modules. */" > OUT
...
END {
    ...
    print "};" > OUT
    print "" > OUT
    print "/* Every importable lib/ module (#692). Sourced from the FILE set, not" > OUT
    print " * from stdlib_docs above -- see the header comment. */" > OUT
    print "static const char *stdlib_modules[] = {" > OUT
    for (i = 1; i <= modules; i++)
        printf "    \"%s\",\n", c_escape(modname[i]) > OUT
    print "    NULL" > OUT
    print "};" > OUT
