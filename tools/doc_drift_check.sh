...
# 4, 6 and 8 MOVED to tools/docs_claims_check.sh (suite [99za]).
# Those three checks each derived one doc NUMBER from the tree (README's stdlib
# headline vs its table, ARCHITECTURE.md's lib/ file count, the builtin counts in
# README.md + docs/BUILTINS.md). Numeric doc claims now have ONE home, and it
# enumerates the whole population instead of the three numbers somebody happened
# to gate: every number followed by a unit word in README.md, docs/llms.txt,
# CLAUDE.md, docs/ARCHITECTURE.md and docs/BUILTINS.md is derived or waived by
# exact line. Do not re-add a number check here.
