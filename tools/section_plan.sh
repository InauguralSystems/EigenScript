...
VERBOSE=1
# Declared populations; changes to runner sections require review here.
CHUNK_EXPECTED=443
SKIP_EMIT_EXPECTED=24
SKIP_ROUTED_EXPECTED=24
...
shard_check() {
    ...
    [ "$total" -gt 0 ] || die "chunk enumeration examined=0 expected=$CHUNK_EXPECTED"
...
skip_audit() {
    ...
    [ "$SKIP_EMIT_EXPECTED" -gt 0 ] && [ "$SP_SKIP_EMITS" -eq "$SKIP_EMIT_EXPECTED" ] || \
        die "skip emitter enumeration examined=$SP_SKIP_EMITS expected=$SKIP_EMIT_EXPECTED (need exact and >0)"

    SP_SKIP_ROUTED=$(grep -cE "$SKIP_ROUTE_RE" "$f")
    [ "$SKIP_ROUTED_EXPECTED" -gt 0 ] && [ "$SP_SKIP_ROUTED" -eq "$SKIP_ROUTED_EXPECTED" ] || \
        die "skip route enumeration examined=$SP_SKIP_ROUTED expected=$SKIP_ROUTED_EXPECTED (need exact and >0)"
...
# One emitted shard runs selected chunks. The parent compares visible section
# headers with visible SECTION_TIME records, then writes the numeric PLAN line.
build_shard_plan() {
    local k="$1" n="$2" s
    ...
    echo "PLAN: shard=$k/$n sections=runtime chunks=$SP_SEL_CHUNKS predicted=${wsec}s unmeasured=$SP_WEIGHT_MISSING"
}

check_chunk_count() {
    local examined
    examined=$(grep -c '[0-9]' "$1")
    [ "$CHUNK_EXPECTED" -gt 0 ] && [ "$examined" -eq "$CHUNK_EXPECTED" ] ||
        die "chunk enumeration examined=$examined expected=$CHUNK_EXPECTED (need exact and >0)"
}
...
selftest() {
    ...
    sed 's/^CHUNK_EXPECTED=443$/CHUNK_EXPECTED=444/' "$0" > "$dir/bad-count.sh"
    chmod +x "$dir/bad-count.sh"
    expect_red 'check_chunk_count: declared population must equal examined > 0' 'chunk enumeration examined' \
    ...
    expect_ok 'control: skip audit examines declared emitters and routes' "$0" --root "$SP_ROOT" --skip-audit
    cp "$RUNNER" "$dir/bad-skip.sh"
    echo 'echo "  SKIP: planted bare emitter"' >> "$dir/bad-skip.sh"
    expect_red 'skip_audit: new SKIP emitter changes exact population' 'skip emitter enumeration' \
        "$0" --root "$SP_ROOT" --runner "$dir/bad-skip.sh" --skip-audit
    sed 's/^SKIP_EMIT_EXPECTED=24$/SKIP_EMIT_EXPECTED=25/' "$0" > "$dir/bad-skip-count.sh"
    chmod +x "$dir/bad-skip-count.sh"
    expect_red 'skip_audit: new bare emitter lacks a reviewed waiver' 'SKIP-emitting line' \
        "$dir/bad-skip-count.sh" --root "$SP_ROOT" --runner "$dir/bad-skip.sh" --skip-audit
    sed 's/^    section_skip "benchmark asset not found (archived)"/    section_skiP "benchmark asset not found (archived)"/' "$RUNNER" > "$dir/bad-route.sh"
    expect_red 'skip_audit: lost section_skip route changes exact population' 'skip route enumeration' \
...
case "$MODE" in
    --chunks)
        ...
        echo "CHUNKS: examined=$CHUNK_EXPECTED expected=$CHUNK_EXPECTED partition=verified" ;;
    --skip-audit)
        W=$(sp_workdir audit); skip_audit "$RUNNER" "$W"
        echo "SKIP AUDIT: emitters examined=$SP_SKIP_EMITS expected=$SKIP_EMIT_EXPECTED; routes examined=$SP_SKIP_ROUTED expected=$SKIP_ROUTED_EXPECTED; waivers=$SP_SKIP_WAIVERS_USED; unaccounted=0" ;;
