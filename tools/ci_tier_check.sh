#!/usr/bin/env bash
# tools/ci_tier_check.sh — platform tiers (#1264), the part the merge queue
...
def cond(x):
    s = str(x).strip(); m = re.fullmatch(r"\$\{\{(.*)\}\}", s, re.S)
...
# (a) every required name: one producer, on pull_request AND merge_group
path = set()
for r in req:
    p = PROD.get(r, [])
    if len(p) != 1:
...
# (d) every ci.yml job is required or a worker of one required always() job
ci = W["ci.yml"][1]
...
selftest() {
    local t pass=0 fail=0; t=$(mktemp -d "${TMPDIR:-/tmp}/ci_tier.XXXXXX") || return 2
    trap 'rm -rf "$t"' RETURN
    ...
    broken() { fail=$((fail + 1)); echo "  FAIL  $1: plant anchor missing (BROKEN, not a pass)"; }
    local CI="$t/w/ci.yml" TS="    name: tsan (concurrency race gate)
"
    fresh; expect "unmodified copy" OK
    fresh; sub "$CI" "$TS" "    name: tsan renamed
" && expect "(a) required job renamed" unproduced || broken rename
    fresh; sub "$t/w/codeql.yml" "  merge_group:" "  workflow_call:" && expect "(a) codeql.yml loses merge_group" not-in-queue || broken queue
    fresh; sub "$t/w/pages.yml" "  pull_request:
    branches: [main]
" "" && expect "(a) pages.yml loses pull_request" not-on-pr || broken pr
